...
def sortie() :
    global note_image, note_video, images, videos, path
    ...
    for i in range(len(note_video)) :
        ...
        sheet1.write(len(note_image)+i,1,os.path.basename(videos[i]))
...
def boutons() :
    ...
    for i in range(len(videos)) :
        ...
        note_video.append(tk.Scale(objet_video[i],from_=0, to=10,orient='horizontal',background="azure",highlightbackground="azure",resolution=1,label="Note",font=("Constantia",12,'italic')))
        note_video[i].grid(padx=5,pady=5,row=i,column=1)
...
path = Path("flora").parent.absolute()
path=str(path)
